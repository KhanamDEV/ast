...
@section('title',$dataview->title.' - Asia School Tours')
@section('description', $dataview->title.' - Asia School Tours')
@section('metatitle', $dataview->title.' - Asia School Tours')
@section('content')
...
<ul class="nav nav-pills" style="font-size: 1rem">
  ...
  <?php 
   ...
   ?>
  <li class="nav-item pr-1">
<a class="nav-link {{ ($valuedataviewtab->slug == $dataview->slug)?'active':' '}}" href="{{url('/safety/'.$valuedataviewtab->slug)}}"  >{{$valuedataviewtab->title}}</a>
  </li>
<?php } ?>
...
<!--  <h2 class="mauxanh">{{$dataview->title}}</h2> -->
  <div class="title-hd-sc pb-1">
    <h5 style="font-weight: 400;line-height: 1.3;letter-spacing: -0.5px;font-size: 16px;">{!! $dataview->body !!}</h5>
    ...
    <div id="collapseTwo" class="collapse" aria-labelledby="headingTwo" data-parent="#accordion">
      <div class="card-body">
            <blockquote class="blockq1uote">
              {!!  str_replace("<>","<br>",$valueold_itine->itinerary_detail ) !!}
              <?php
       ...
       <!-- 
        <div class="wr-img-1" style="position: relative;">
          <img src="{{url('img/about2.png')}}" alt="">
           <div class="wwwaa" style="position: absolute;top: 50px;right: 0px;">
